   ...
   public Card(int value, String name, String suit){
		...
		if(suit==null) {
			...
			if( suit.equals("hearts") ){
				try{
					suitImage = ImageIO.read(new File("heartsrealreal.png"));
					
				} catch (IOException e) {}
			}
			if(suit.equals("diamonds")) {
				try{
					suitImage = ImageIO.read(new File("diamondrealrealreal.png"));
					
				} catch (IOException e) {}
			}
			if(suit.equals("spades")) {
				try{
					suitImage = ImageIO.read(new File("spadesreal.png"));
					
				} catch (IOException e) {}
			}
			if(suit.equals("clubs")) {
				try{
					suitImage = ImageIO.read(new File("clubsreal.png"));
					
				} catch (IOException e) {}
			}
		}
		try {
			cardImage=ImageIO.read(new File("cardreal.png"));
		}catch(IOException e) {
	 ...
	 public void drawMe(Graphics g, int x, int y){
      if(selected) {
    	  ...
    	  g.fillRect(x+15, y+5, 80, 130);
    	  g.setColor(new Color(225,225,225));
    	  g.fillRect(x+15,y+5,10,10);
    	  g.fillRect(x+85,y+5,10,10);
    	  g.fillRect(x+15,y+125,10,10);
    	  g.fillRect(x+85,y+125,10,10);
      }else {
