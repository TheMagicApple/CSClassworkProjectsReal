...
public class MinHeap<E extends Comparable<E>> {
	private MyArrayList<E> myList=new MyArrayList<>();
	public void add(E e) {
		myList.add(e);
		...
		while(i<myList.size()) {
			...
			if(right>=myList.size()) {
				if(left>=myList.size()) {
					...
					if(myList.get(left)!=null && myList.get(left).compareTo(myList.get(i))<0) {
						int swap=left;
						E temp=myList.get(i);
			...
			if(myList.get(left)!=null && myList.get(left).compareTo(myList.get(i))<0 && myList.get(right).compareTo(myList.get(i))<0) {
